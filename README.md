...
[downloads.programs.analysis]
product = "tapio-analysis"           # a configured product; optional when there is one
slug = "tapio-analysis"              # bucket prefix and file-name prefix: [a-z0-9-]
name = "Tapio Analysis"
...
[[downloads.programs.analysis.assets]]
os = "windows"                       # windows | linux | macos
arch = "x86_64"                      # x86_64 | aarch64; default x86_64
glob = "dist/tapio-analysis-${version}.exe"   # ${version} is the tag; exactly one match
archive = "none"                     # zip | tar.gz | none; default zip on Windows, tar.gz elsewhere
sbom = "dist/tapio-analysis-${version}.cdx.json"   # optional; linked from the page
```
...
tapio-build --project . downloads render --program analysis \
...
source archive is read from it). Outputs: `published`, `prerelease`, `latest`,
